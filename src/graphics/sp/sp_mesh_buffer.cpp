...
void SPMeshBuffer::recreateVAO(unsigned i)
{
    ...
    if (CVS->isARBBufferStorageUsable())
    {
        glBufferStorage(GL_ARRAY_BUFFER,
            m_gl_instance_size[i] * SPInstancedData::getStride(), NULL,
            GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT | GL_MAP_COHERENT_BIT);
        m_ins_dat_mapped_ptr[i] = glMapBufferRange(GL_ARRAY_BUFFER, 0,
            m_gl_instance_size[i] * SPInstancedData::getStride(),
            GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT | GL_MAP_COHERENT_BIT);
    }
    else
#endif
    ...
    glVertexAttribPointer(8, 3, GL_FLOAT, GL_FALSE,
        (GLsizei)SPInstancedData::getStride(), (void*)0);
    glVertexAttribDivisor(8, 1);
    // Rotation (quaternion in 4 32bit floats)
    glEnableVertexAttribArray(9);
    glVertexAttribPointer(9, 4, GL_FLOAT, GL_FALSE,
        (GLsizei)SPInstancedData::getStride(), (void*)12);
    glVertexAttribDivisor(9, 1);
    // Scale (3 half floats and .w unused)
    glEnableVertexAttribArray(10);
    glVertexAttribPointer(10, 4, GL_HALF_FLOAT, GL_FALSE,
        (GLsizei)SPInstancedData::getStride(), (void*)28);
    glVertexAttribDivisor(10, 1);
    // Texture translation
    glEnableVertexAttribArray(11);
    glVertexAttribPointer(11, 2, GL_SHORT, GL_TRUE,
        (GLsizei)SPInstancedData::getStride(), (void*)36);
    glVertexAttribDivisor(11, 1);
    // Misc data (skinning offset and hue change)
    glEnableVertexAttribArray(12);
    glVertexAttribIPointer(12, 2, GL_SHORT,
        (GLsizei)SPInstancedData::getStride(), (void*)40);
    glVertexAttribDivisor(12, 1);
    // World-space velocity for relativistic apparent-position effects
    glEnableVertexAttribArray(13);
    glVertexAttribPointer(13, 3, GL_FLOAT, GL_FALSE,
        (GLsizei)SPInstancedData::getStride(), (void*)44);
