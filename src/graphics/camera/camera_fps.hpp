...
//  Copyright (C) 2006-2015 MinkowskiKart-Team, Steve Baker
//
...
private:
    ...
    /** Default local observer position used when recentering the view. */
    core::vector3df m_default_local_position;

    /** Default local observer direction used when recentering the view. */
    core::vector3df m_default_local_direction;

    /** Default local observer up vector used when recentering the view. */
    core::vector3df m_default_local_up;

    /** Hide the attached kart body so the observer camera stays readable. */
    bool m_hide_attached_kart_body;

    /** Whether mouse motion should steer this FPS camera. */
    bool m_mouse_look_enabled;

    /** Keep attached observer motion aligned to world-up instead of kart pitch/roll. */
    bool m_stabilize_attached_camera;

    /** Smoothed forward direction for stabilized observer mode. */
    core::vector3df m_attached_forward;
...
public:
    ...
    virtual void setInitialTransform() OVERRIDE;
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    /** Sets the local observer rig used while attached to a kart. */
    void setLocalObserverTransform(const core::vector3df& position,
                                   const core::vector3df& direction,
                                   const core::vector3df& up);

    // ------------------------------------------------------------------------
    /** Hide or restore the attached kart body. */
    void setHideAttachedKartBody(bool value);

    // ------------------------------------------------------------------------
    /** Enable or disable mouse-look for this FPS camera. */
    void setMouseLookEnabled(bool value) { m_mouse_look_enabled = value; }

    // ------------------------------------------------------------------------
    /** Whether this FPS camera should consume mouse-look input. */
    bool isMouseLookEnabled() const { return m_mouse_look_enabled; }

    // ------------------------------------------------------------------------
    /** Reset the current FPS view back to the configured observer transform. */
    void recenterView();

    // ------------------------------------------------------------------------
    /** Keep an attached observer camera stabilized to world-up. */
    void setStabilizeAttachedCamera(bool value)
    {
        m_stabilize_attached_camera = value;
    }
