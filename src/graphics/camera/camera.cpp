...
void configureRelativisticObserverCamera(Camera* camera)
{
    if (!camera || camera->getType() != Camera::CM_TYPE_FPS ||
        !Relativity::shouldUseFirstPersonObserverCamera())
    {
        return;
    }

    CameraFPS* fps = dynamic_cast<CameraFPS*>(camera);
    AbstractKart* kart = camera->getKart();
    if (!fps || !kart)
        return;

    const float observer_height = std::max(0.55f,
        std::max(kart->getHighestPoint() * 0.72f,
                 kart->getKartHeight() * 0.65f));
    const float observer_forward = std::max(0.35f,
        kart->getKartLength() * 0.34f);

    fps->setAttachedFpsCam(true);
    fps->setSmoothMovement(false);
    fps->setMouseLookEnabled(true);
    fps->setStabilizeAttachedCamera(true);
    fps->setLinearVelocity(core::vector3df(0.0f, 0.0f, 0.0f));
    fps->setAngularVelocity(0.0f);
    fps->setLocalObserverTransform(core::vector3df(0.0f, observer_height,
                                                   observer_forward),
                                   core::vector3df(0.0f, -0.03f, 1.0f),
                                   core::vector3df(0.0f, 1.0f, 0.0f));
    fps->setHideAttachedKartBody(true);
    fps->setInitialTransform();
}   // configureRelativisticObserverCamera
                             ...
                             AbstractKart* kart)
{
    ...
    configureRelativisticObserverCamera(camera);
    return camera;
}   // createCamera
...
void Camera::changeCamera(unsigned int camera_index, CameraType type)
{
    ...
    const bool was_active = s_active_camera == old_camera;
