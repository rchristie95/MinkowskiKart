...
namespace SkinConfig
{
    ...
    static std::string resolveBundledTTF(const std::string& name)
    {
        const std::string resolved =
            file_manager->getAssetChecked(FileManager::TTF, name, false);
        if (!resolved.empty())
        {
            Log::debug("skin", "Resolved bundled TTF '%s' to '%s'.",
                       name.c_str(), resolved.c_str());
            return resolved;
        }

        const std::string candidate = file_manager->getAsset(FileManager::TTF,
                                                             name);
        Log::fatal("skin", "Missing bundled TTF '%s' (candidate '%s', asset "
                   "directory '%s').", name.c_str(), candidate.c_str(),
                   file_manager->getAssetDirectory(FileManager::TTF).c_str());
        return "";
    }   // resolveBundledTTF
    ...
    static void loadFromFile(std::string file, bool clear_prev_params, std::vector<std::string>& skin_paths)
    {
        ...
        if (clear_prev_params)
        {
            ...
            for (auto& p : stk_config->m_normal_ttf)
                m_normal_ttf.push_back(resolveBundledTTF(p));
            m_digit_ttf.clear();
            for (auto& p : stk_config->m_digit_ttf)
                m_digit_ttf.push_back(resolveBundledTTF(p));
            m_color_emoji_ttf.clear();
            if (!stk_config->m_color_emoji_ttf.empty())
            {
                m_color_emoji_ttf =
                    resolveBundledTTF(stk_config->m_color_emoji_ttf);
            }
