...
EventPropagation InputManager::input(const SEvent& event)
{
    ...
    auto recenter_active_fps_camera = [this]() -> bool
    {
        CameraFPS* cam = dynamic_cast<CameraFPS*>(Camera::getActiveCamera());
        const bool can_recenter =
            cam != NULL &&
            cam->isMouseLookEnabled() &&
            StateManager::get()->getGameState() == GUIEngine::GAME &&
            !GUIEngine::ModalDialog::isADialogActive() &&
            !GUIEngine::ScreenKeyboard::isActive();
        if (!can_recenter)
        ...
        if (device)
        {
            ...
            const int mid_x = (int)screen_size.Width / 2;
            const int mid_y = (int)screen_size.Height / 2;
            device->getCursorControl()->setPosition(mid_x, mid_y);
            m_mouse_val_x = mid_x;
            m_mouse_val_y = mid_y;
        }
        else
    ...
    else if (event.EventType == EET_KEY_INPUT_EVENT)
    {
        ...
        {
            if (key == IRR_KEY_G && recenter_active_fps_camera())
                return EVENT_BLOCK;
            const bool is_relativity_decrease =
                key == IRR_KEY_MINUS || event.KeyInput.Char == '-';
            const bool is_relativity_increase =
                key == IRR_KEY_PLUS ||
                event.KeyInput.Char == '=' ||
                event.KeyInput.Char == '+';
    ...
    else if (event.EventType == EET_MOUSE_INPUT_EVENT)
    {
        ...
        CameraFPS *cam = dynamic_cast<CameraFPS*>(Camera::getActiveCamera());
        const bool should_capture_fps_mouse =
            cam != NULL &&
            cam->isMouseLookEnabled() &&
            StateManager::get()->getGameState() == GUIEngine::GAME &&
            !GUIEngine::ModalDialog::isADialogActive() &&
        ...
        if ((type == EMIE_LMOUSE_DOUBLE_CLICK ||
             type == EMIE_RMOUSE_DOUBLE_CLICK ||
             type == EMIE_MMOUSE_DOUBLE_CLICK) &&
            recenter_active_fps_camera())
        {
            return EVENT_BLOCK;
        }
        ...
        case MENU:
            ...
            switch (m_mode)
            {
                ...
                case INPUT_SENSE_GAMEPAD:
                    ...
                    // to make MK multithreaded: m_sensed_input must not be
