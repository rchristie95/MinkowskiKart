...
//  Copyright (C) 2006-2015 Joerg Henrichs, MinkowskiKart-Team, Steve Baker
//
...
struct RelativisticLapHUDState
{
    int    m_last_finished_lap;
    double m_lap_start_proper_time_s;

    RelativisticLapHUDState()
        : m_last_finished_lap(-1),
          m_lap_start_proper_time_s(0.0)
    {
    }
};
...
double getCurrentLapProperTime(const AbstractKart* kart, int finished_laps,
                               double total_proper_time_s)
{
    RelativisticLapHUDState &state =
        g_relativistic_lap_hud_state[kart->getWorldKartId()];

    if (finished_laps < state.m_last_finished_lap ||
        total_proper_time_s < state.m_lap_start_proper_time_s)
    {
        state.m_last_finished_lap = finished_laps;
        state.m_lap_start_proper_time_s = 0.0;
    }

    if (finished_laps > state.m_last_finished_lap)
    {
        state.m_last_finished_lap = finished_laps;
        state.m_lap_start_proper_time_s = total_proper_time_s;
    }
...
void RaceGUI::drawGlobalMiniMap()
{
    ...
    for (unsigned int i = 0; i < karts.size(); i++)
    {
        ...
        const bool is_local = is_nw_spectate ? kart == target_kart :
            kart->getController()->isLocalPlayerController();

        // don't draw eliminated kart
        if (kart->isEliminated() && !(sta && sta->isMoving()))
            continue;
        if (!kart->isVisible() && !is_local)
            continue;
                      ...
                      const core::vector2df &scaling)
{
    ...
    if (lap + 1> RaceManager::get()->getNumLaps())
        lap--;
    const bool show_lap_counter = lap >= 0;
    ...
    {
        const Kart* sr_kart = dynamic_cast<const Kart*>(kart);
        if (sr_kart)
        {
