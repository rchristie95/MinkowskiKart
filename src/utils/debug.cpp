...
bool handleContextMenuAction(s32 cmd_id)
{
    ...
    case DEBUG_POWERUP_WORMHOLE:
        ...
        break;
    case DEBUG_POWERUP_ASTEROID:
        addPowerup(PowerupManager::POWERUP_ASTEROID, 255);
        break;
...
void handleStaticAction(int key, int value, bool control_pressed, bool shift_pressed)
{
    World* world = World::getWorld();
    CameraFPS *cam = dynamic_cast<CameraFPS*>(Camera::getActiveCamera());
    const bool allow_free_fps_debug_motion =
        cam != NULL && !cam->getAttachedFpsCam();
    ...
    if (value)
    {
        switch (key)
        {
            ...
            case IRR_KEY_F5:
            {
                ...
                {
                    handleContextMenuAction(DEBUG_POWERUP_ASTEROID);
                }
                break;
            }
            case IRR_KEY_F6:
    ...
    switch (key)
    {
        ...
        case IRR_KEY_W:
        {
            if (allow_free_fps_debug_motion)
            {
