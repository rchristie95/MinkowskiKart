...
void CameraFPS::update(float dt)
{
    ...
    // The attached observer view needs a smaller near plane so nearby
    // geometry does not vanish as the camera sits close to the track.
    m_camera->setNearValue(m_attached ? 0.1f : 1.0f);
...
void CameraFPS::setInitialTransform()
{
    ...
    btTransform t = m_kart->getSmoothedTrans();
    Vec3 local_position(m_local_position.X, m_local_position.Y, m_local_position.Z);
    core::vector3df pos = Vec3(t(local_position)).toIrrVector();

    btQuaternion q = t.getRotation();
    btMatrix3x3 mat(q);
    Vec3 local_direction(m_local_direction.X, m_local_direction.Y, m_local_direction.Z);
    Vec3 local_up(m_local_up.X, m_local_up.Y, m_local_up.Z);
    core::vector3df direction = Vec3(mat * local_direction).toIrrVector();
    core::vector3df up = Vec3(mat * local_up).toIrrVector();
