...
void World::reset(bool restart)
{
    ...
    if (!GUIEngine::isNoGraphics())
    {
        ...
        if (Relativity::shouldUseFirstPersonObserverCamera())
        {
            const unsigned int local_players =
                RaceManager::get()->getNumLocalPlayers();
            for (unsigned int i = 0; i < local_players &&
                 i < Camera::getNumCameras(); i++)
            {
                Camera::changeCamera(i, Camera::CM_TYPE_FPS);
            }
        }
    }
