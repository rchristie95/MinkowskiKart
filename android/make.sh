...
export APP_NAME_BETA="MinkowskiKart Beta"
...
export APP_NAME_DEBUG="MinkowskiKart"
...
# Use `magick` when it's available, or define a fallback wrapper
if command -v magick > /dev/null; then
    MAGICK='magick'
    magick_convert() { magick "$@"; }
elif command -v convert > /dev/null && convert -version 2>/dev/null | grep -q "ImageMagick"; then
    MAGICK='convert'
    magick_convert() { convert "$@"; }
else
    echo "Warning: ImageMagick (magick/convert) not found. Using copy fallback for icons."
    magick_convert() {
        local infile="$1"
        local outfile="${@: -1}"
        if [[ "$infile" =~ ^xc: ]]; then
...
PO_EXTRACT_ERROR_MSG_STR="Check remaining device space or reinstall MinkowskiKart."
PO_QUIT="po_quit"
PO_QUIT_STR="Quit"
...
magick_convert "$APP_ICON" -scale 48x48 "$DIRNAME/res/drawable-mdpi/icon.png"
magick_convert "$APP_ICON" -scale 72x72 "$DIRNAME/res/drawable-hdpi/icon.png"
...
magick_convert "$APP_ICON_ADAPTIVE_FG" -scale 216x216 "$DIRNAME/res/drawable-xhdpi/icon_fg.png"
magick_convert "$APP_ICON_ADAPTIVE_FG" -scale 324x324 "$DIRNAME/res/drawable-xxhdpi/icon_fg.png"
magick_convert "$APP_ICON_ADAPTIVE_FG" -scale 432x432 "$DIRNAME/res/drawable-xxxhdpi/icon_fg.png"
